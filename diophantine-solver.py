...
def diophantine_solver(coefficients, right_hand_side):
...
# Make all solutions represented by new variables.
  ...
  while column < row_counter:
    ...
    while row < row_counter:
      if sys_of_eq[row, column] == 1:
        ...
        sys_of_eq[:, [constant_column, constant_column +
                      1]] = sys_of_eq[:,
                                      [constant_column + 1, constant_column]]
        constant_column += 1

        sys_of_eq[[column, row_counter], :] = sys_of_eq[
          [row_counter, column], :]
  ...
  for counter in numpy.arange(0, _dim - 1):
    ...
    sys_of_eq[:, [counter,
                  int(numpy.nonzero(non_zero_entries)[0]) +
                  counter]] = sys_of_eq[:, [
                    int(numpy.nonzero(non_zero_entries)[0]) + counter, counter
                  ]]
